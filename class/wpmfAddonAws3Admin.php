...
class WpmfAddonAws3Admin
{
    ...
    public function handleAjax()
    {
        ...
        add_action('wp_ajax_wpmf-import-objects-from-bucket', array($this, 'importObjectsFromBucket'));
     ...
     * @return void
     */
    public function importObjectsFromBucket()
    {
        ...
        set_time_limit(0);
        $arrs = get_option('wpmf_s3_import_list', true);
        if (empty($arrs)) {
        ...
        $aws3config = get_option('_wpmfAddon_aws3_config');
        // get first element
        $import = array_shift($arrs);
        if ($import['type'] === 'folder') {
            $inserted = wp_insert_term(basename($import['key']), WPMF_TAXO, array('parent' => $import['parent']));
            ...
            $exist = $wpdb->get_var($wpdb->prepare('SELECT COUNT(meta_id) FROM ' . $wpdb->postmeta . ' WHERE meta_key = "wpmf_s3_import_key" AND meta_value = %s', array($aws3config['bucket'] . '/' . $import['key'])));
            if (empty($exist)) {
                // don't upload to S3
                ...
                $file_path = $upload_dir['path'] . '/' . basename($import['key']);
                $file_url = $upload_dir['url'] . '/' . basename($import['key']);
                ...
                try {
                    ...
                    wp_set_object_terms((int) $attach_id, (int) $import['parent'], WPMF_TAXO, false);
                    $attach_data = wp_generate_attachment_metadata($attach_id, $file_path);
                    wp_update_attachment_metadata($attach_id, $attach_data);
                    update_post_meta($attach_id, 'wpmf_s3_import_key', $bucket . '/' . $import['key']);
                } catch (S3Exception $e) {
                    update_option('wpmf_s3_import_list', $arrs);
                    sleep(1);
        ...
        if ($import['type'] === 'folder') {
            ...
            global $old_parent, $new_parent;
            $old_parent = $import['key'];
            $new_parent = $parent;

            $arrs = array_map(
                function ($value) {
                    global $old_parent, $new_parent;
                    if ($value['parent'] === $old_parent) {
    ...
    public function listAllObjectsFromBucket()
    {
        ...
        if (isset($aws3config['bucket'])) {
            ...
            $new_list = $this->extractListObjectsFromPath($list, $term_root_id);
            ksort($new_list);
            update_option('wpmf_s3_import_list', $new_list);
...
' . esc_sql($column) . ' REGEXP \'s:[0-9]+:".*(' . esc_sql(preg_quote($search_url)) . '|' . esc_sql(preg_quote($attached_file)) . ').*";\'';
    ...
    public function removeFilesAfterUpload()
    {
        ...
        global $wpdb;
        $limit     = 5;
        $s3_queues = $wpdb->get_results($wpdb->prepare('SELECT * FROM ' . $wpdb->prefix . 'wpmf_s3_queue WHERE status=0 ORDER BY id DESC LIMIT %d', array($limit)));
        if (empty($s3_queues)) {
            wp_send_json(array('status' => true));
        }
    ...
    public function removeFilesFromServer()
    {
        ...
        // update database
        global $wpdb;
        $offset = ((int) $_POST['page'] - 1) * 10;
        $files = $wpdb->get_results($wpdb->prepare('SELECT * FROM ' . $wpdb->posts . ' as p INNER JOIN ' . $wpdb->postmeta . ' as pm ON p.ID = pm.post_id WHERE p.post_type = "attachment" AND pm.meta_key = "wpmf_awsS3_info" AND pm.meta_value !="" LIMIT 10 OFFSET %d', $offset));
        if (empty($files)) {
            wp_send_json(array('status' => true, 'continue' => false));
        }

        foreach ($files as $file) {
            $this->doRemoveLocalFile($file->ID);
        }
    ...
    public function downloadObject()
    {
        ...
        set_time_limit(0);
        global $wpdb;
        $file = $wpdb->get_row('SELECT * FROM ' . $wpdb->prefix . 'wpmf_s3_queue WHERE status = 1 OR status = 2 LIMIT 1');
        if ($file) {
            try {
                $aws3 = new WpmfAddonAWS3();
    ...
    public function uploadToS3()
    {
        ...
        try {
            ...
            foreach ($attachments as $attachment) {
                ...
                if (in_array($filetype['ext'], $allow_exts_array)) {
    ...
    public function wpGetAttachmentUrl($url, $post_id)
    {
        ...
        if (!empty($infos)) {
            ...
            if (!empty($aws3config['enable_custom_domain']) && !empty($aws3config['custom_domain'])) {
                return esc_url($aws3config['custom_domain'] . '/' . $infos['Key']);
            } else {
                if (isset($infos['Region']) && $infos['Region'] !== 'us-east-1') {
                    return 'https://s3-' . $infos['Region'] . '.amazonaws.com/' . $infos['Bucket'] . '/' . $infos['Key'];
                } else {
                    return 'https://s3.amazonaws.com/' . $infos['Bucket'] . '/' . $infos['Key'];
                }
            }
        }
