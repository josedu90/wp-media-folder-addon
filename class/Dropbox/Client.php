...
class Client
{
    ...
    function getFile($path, $outStream, $rev = null)
    {
        ...
        $url = RequestUtil::buildUri($this->contentHost,"2/files/download");       
        $curl = $this->mkCurl($url);
       
        $params = array("path" => $path,"rev" => $rev);
     ...
     * @var int
     */
    private static $AUTO_CHUNKED_UPLOAD_THRESHOLD = 157286400;  // 8 MB
    ...
    function updateFile($path,$rev, $writeMode, $inStream, $numBytes = null)
    {
        Path::checkArgNonRoot("path", $path);
        WriteMode::checkArg("writeMode", $writeMode);
        Checker::argResource("inStream", $inStream);
        ...
        // In all other cases, use regular upload.
        if ($numBytes === null || $numBytes > self::$AUTO_CHUNKED_UPLOAD_THRESHOLD) {
            $metadata = $this->_updateFileChunked($path, $writeMode, $inStream, $numBytes,
                                                  self::$DEFAULT_CHUNK_SIZE);
        } else {
            $metadata = $this->_updateFile($path,$rev, $writeMode,
                function(Curl $curl) use ($inStream, $numBytes) {
                    $curl->set(CURLOPT_PUT, true);
                    $curl->set(CURLOPT_CUSTOMREQUEST, 'POST');                    
                    $curl->set(CURLOPT_INFILE, $inStream);
    ...
    private function _uploadFile($path, $writeMode, $curlConfigClosure)
    {
        ...
        $url = "https://" . $this->contentHost . "/2/files/upload";       
        $curl = $this->mkCurl($url);       
        $params = array(
          "path"  => $path,
          "mode"  => "add"         
    ...
    private function _updateFile($path,$rev, $writeMode, $curlConfigClosure)
    {
        Path::checkArg("path", $path);
        WriteMode::checkArg("writeMode", $writeMode);
        Checker::argCallable("curlConfigClosure", $curlConfigClosure);       
        ...
        $params = array(
          "path"  => $path,
          "mode"  => array(".tag"=>"update","update"=>$rev ), 
          "autorename" => false,
          "mute" => false 
        );
    ...
    {
        $params = array("path"=> $path, "include_media_info" => false,"include_deleted" => false);
        $response = $this->doPost(
            $this->apiHost,
            "2/files/get_metadata", array(),
            $params);
     ...
     * @throws Exception
     */
    function getMetadataWithChildren($path,$recursive = false)
    {       
        $params = array("path"=> $path, "recursive"=> $recursive,"include_deleted"=>false,"include_media_info"=>true);
        $response = $this->doPost(
            $this->apiHost,
            "2/files/list_folder", array(),
            $params);
    ...
    {
        $params = array("path"=> $path, "include_deleted"=>false,"include_media_info"=>true);
        $response = $this->doPost(
            $this->apiHost,
    ...
    private function _getMetadata($path, $params)
    {
        $response = $this->doGet(
            $this->apiHost,
            $this->appendFilePath("2/files/get_metadata", $path),
            $params);
    ...
    function listRevisions($path,$limit = null)
    {
        $params = array("path"=> $path,"limit" => $limit);
        $response = $this->doPost(
            $this->apiHost,
            "2/files/list_revisions", array(),
            $params);

        if ($response->statusCode === 404) return null;
        if ($response->statusCode !== 200) throw RequestUtil::unexpectedStatus($response);

        $listRevisions = RequestUtil::parseResponseJson($response->body);
        if (array_key_exists("is_deleted", $listRevisions) && $listRevisions["is_deleted"]) return null;
    ...
    function restoreFile($path, $rev)
    {
        ...
        $response = $this->doPost(
            $this->apiHost,
            "2/files/restore", array(),
            $params);

        if ($response->statusCode === 404) return null;
        if ($response->statusCode !== 200) throw RequestUtil::unexpectedStatus($response);

        $restoreRevision = RequestUtil::parseResponseJson($response->body);
        if (array_key_exists("is_deleted", $restoreRevision) && $restoreRevision["is_deleted"]) return null;
    ...
    function searchFileNames($basePath, $query)
    {
        ...
        $results = RequestUtil::parseResponseJson($response->body);
        if (array_key_exists("is_deleted", $results) && $results["is_deleted"]) return null;
        return $results;
    }
    ...
    function create_shared_link($path){
        Path::checkArgNonRoot("path", $path);
        $params = array(
            "path"  => $path,
            "short_url" => false
    ...
    function get_shared_links($path){
        Path::checkArgNonRoot("path", $path);
        $params = array(
            "path"  => $path
        );
    ...
    function get_filecontent($path){
        Path::checkArgNonRoot("path", $path);
        $url = "https://" . $this->contentHost . "/2/files/download"; 
        $curl = $this->mkCurl($url);     
        $params = array(
            "path"  => $path
        );
     ...
     * @throws Exception
     */    
    function getThumbnail($path, $format, $size,$mimetype)
    {
        ...
        if (!in_array($size, array("w32h32", "w64h64", "w128h128", "w640h480", "w1024h768"))) {
            throw new \InvalidArgumentException("Invalid 'size': ".Util::q($format));
        }
             
        $url = "https://" . $this->contentHost . "/2/files/get_thumbnail"; 
        $curl = $this->mkCurl($url);     
        $params = array(
            "path"  => $path,
            "format" => $format,
     ...
     * @throws Exception
     */
    function doPost($host, $path, $headers= array(), $params = null)
    {
        Checker::argString("host", $host);
        Checker::argString("path", $path);
        return RequestUtil::doPost($this->clientIdentifier, $this->accessToken, 
                                   $host, $path,$headers, $params);
    }
