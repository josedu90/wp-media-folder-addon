...
class FnDispatcher
{
    ...
    private function fn_sort(array $args)
    {
        ...
        $valid = ['string', 'number'];
        return \WP_Media_Folder\JmesPath\Utils::stableSort($args[0], function ($a, $b) use($valid) {
    ...
    private function fn_sort_by(array $args)
    {
        ...
        $expr = $args[1];
        $valid = ['string', 'number'];
        return \WP_Media_Folder\JmesPath\Utils::stableSort($args[0], function ($a, $b) use($expr, $valid) {
            $va = $expr($a);
            $vb = $expr($b);
    ...
    private function fn_type(array $args)
    {
        ...
        return \WP_Media_Folder\JmesPath\Utils::type($args[0]);
    }
    ...
    private function fn_to_number(array $args)
    {
        ...
        $value = $args[0];
        $type = \WP_Media_Folder\JmesPath\Utils::type($value);
        if ($type == 'number') {
            return $value;
    ...
    private function fn_to_array(array $args)
    {
        ...
        return \WP_Media_Folder\JmesPath\Utils::isArray($args[0]) ? $args[0] : [$args[0]];
    }
    ...
    {
        if ($types[0] == 'any' || in_array(\WP_Media_Folder\JmesPath\Utils::type($value), $types) || $value === [] && in_array('object', $types)) {
            return;
        }
        $msg = 'must be one of the following types: ' . implode(', ', $types) . '. ' . \WP_Media_Folder\JmesPath\Utils::type($value) . ' found';
    ...
    {
        $ta = \WP_Media_Folder\JmesPath\Utils::type($a);
        $tb = \WP_Media_Folder\JmesPath\Utils::type($b);
        if ($ta !== $tb) {
