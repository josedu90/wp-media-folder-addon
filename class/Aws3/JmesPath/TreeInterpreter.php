...
class TreeInterpreter
{
    ...
    {
        $this->fnDispatcher = $fnDispatcher ?: \WP_Media_Folder\JmesPath\FnDispatcher::getInstance();
    }
    /**
    ...
    private function dispatch(array $node, $value)
    {
        ...
        switch ($node['type']) {
            ...
            case 'index':
                if (!\WP_Media_Folder\JmesPath\Utils::isArray($value)) {
                    return null;
                }
            ...
            case 'projection':
                ...
                switch ($node['from']) {
                    case 'object':
                        if (!\WP_Media_Folder\JmesPath\Utils::isObject($left)) {
                            return null;
                        }
                        break;
                    case 'array':
                        if (!\WP_Media_Folder\JmesPath\Utils::isArray($left)) {
                            return null;
                        }
                        break;
                    default:
            ...
            case 'or':
                ...
                return \WP_Media_Folder\JmesPath\Utils::isTruthy($result) ? $result : $this->dispatch($node['children'][1], $value);
            case 'and':
                $result = $this->dispatch($node['children'][0], $value);
                return \WP_Media_Folder\JmesPath\Utils::isTruthy($result) ? $this->dispatch($node['children'][1], $value) : $result;
            case 'not':
                return !\WP_Media_Folder\JmesPath\Utils::isTruthy($this->dispatch($node['children'][0], $value));
            case 'pipe':
                return $this->dispatch($node['children'][1], $this->dispatch($node['children'][0], $value));
            ...
            case 'comparator':
                ...
                if ($node['value'] == '==') {
                    return \WP_Media_Folder\JmesPath\Utils::isEqual($left, $right);
                } elseif ($node['value'] == '!=') {
                    return !\WP_Media_Folder\JmesPath\Utils::isEqual($left, $right);
                } else {
            ...
            case 'condition':
                return \WP_Media_Folder\JmesPath\Utils::isTruthy($this->dispatch($node['children'][0], $value)) ? $this->dispatch($node['children'][1], $value) : null;
            case 'function':
                $args = [];
            ...
            case 'slice':
                return is_string($value) || \WP_Media_Folder\JmesPath\Utils::isArray($value) ? \WP_Media_Folder\JmesPath\Utils::slice($value, $node['value'][0], $node['value'][1], $node['value'][2]) : null;
            case 'expref':
