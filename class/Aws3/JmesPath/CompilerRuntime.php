...
class CompilerRuntime
{
     ...
     */
    public function __construct($dir = null, \WP_Media_Folder\JmesPath\Parser $parser = null)
    {
        $this->parser = $parser ?: new \WP_Media_Folder\JmesPath\Parser();
        $this->compiler = new \WP_Media_Folder\JmesPath\TreeCompiler();
        $dir = $dir ?: sys_get_temp_dir();
        if (!is_dir($dir) && !mkdir($dir, 0755, true)) {
            throw new \RuntimeException("Unable to create cache directory: {$dir}");
        }
        $this->cacheDir = realpath($dir);
        $this->interpreter = new \WP_Media_Folder\JmesPath\TreeInterpreter();
    }
    /**
