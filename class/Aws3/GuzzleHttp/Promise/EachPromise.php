 ...
 */
class EachPromise implements \WP_Media_Folder\GuzzleHttp\Promise\PromisorInterface
{
    private $pending = [];
    /** @var \Iterator */
    private $iterable;
    ...
    {
        $this->mutex = false;
        $this->aggregate = new \WP_Media_Folder\GuzzleHttp\Promise\Promise(function () {
            reset($this->pending);
            ...
            while ($promise = current($this->pending)) {
                next($this->pending);
                $promise->wait();
                if ($this->aggregate->getState() !== \WP_Media_Folder\GuzzleHttp\Promise\PromiseInterface::PENDING) {
                    return;
                }
            }
        });
