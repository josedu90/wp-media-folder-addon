...
class MessageFormatter
{
     ...
     * @return string
     */
    public function format(\WP_Media_Folder\Psr\Http\Message\RequestInterface $request, \WP_Media_Folder\Psr\Http\Message\ResponseInterface $response = null, \Exception $error = null)
    {
        $cache = [];
        return preg_replace_callback('/{\\s*([A-Za-z_\\-\\.0-9]+)\\s*}/', function (array $matches) use($request, $response, $error, &$cache) {
            ...
            switch ($matches[1]) {
                case 'request':
                    $result = \WP_Media_Folder\GuzzleHttp\Psr7\str($request);
                    break;
                case 'response':
                    $result = $response ? \WP_Media_Folder\GuzzleHttp\Psr7\str($response) : '';
                    break;
                case 'req_headers':
    ...
    private function headers(\WP_Media_Folder\Psr\Http\Message\MessageInterface $message)
    {
        $result = '';
