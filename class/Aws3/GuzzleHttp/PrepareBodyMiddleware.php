    ...
    public function __invoke(\WP_Media_Folder\Psr\Http\Message\RequestInterface $request, array $options)
    {
        ...
        if (!$request->hasHeader('Content-Type')) {
            if ($uri = $request->getBody()->getMetadata('uri')) {
                if ($type = \WP_Media_Folder\GuzzleHttp\Psr7\mimetype_from_filename($uri)) {
        ...
        return $fn(\WP_Media_Folder\GuzzleHttp\Psr7\modify_request($request, $modify), $options);
    }
    private function addExpectHeader(\WP_Media_Folder\Psr\Http\Message\RequestInterface $request, array $options, array &$modify)
    {
