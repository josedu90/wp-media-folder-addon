 ...
 */
class Waiter implements \WP_Media_Folder\GuzzleHttp\Promise\PromisorInterface
{
     ...
     */
    public function __construct(\WP_Media_Folder\Aws\AwsClientInterface $client, $name, array $args = [], array $config = [])
    {
        $this->client = $client;
        $this->name = $name;
        $this->args = $args;
        ...
        return \WP_Media_Folder\GuzzleHttp\Promise\coroutine(function () {
            ...
            for ($state = 'retry', $attempt = 1; $state === 'retry'; $attempt++) {
                ...
                if ($state === 'success') {
                    ...
                    (yield new \WP_Media_Folder\GuzzleHttp\Promise\RejectedPromise(new \RuntimeException($msg)));
                } elseif ($state === 'retry' && $attempt >= $this->config['maxAttempts']) {
                    $state = 'failed';
                    (yield new \WP_Media_Folder\GuzzleHttp\Promise\RejectedPromise(new \RuntimeException("The {$this->name} waiter failed after attempt #{$attempt}.")));
                }
            }
        });
    }
