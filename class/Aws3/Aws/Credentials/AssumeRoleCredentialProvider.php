...
use WP_Media_Folder\Aws\Sts\StsClient;
...
class AssumeRoleCredentialProvider
{
    ...
    {
        $client = $this->client;
        return $client->assumeRoleAsync($this->assumeRoleParams)->then(function (\WP_Media_Folder\Aws\Result $result) {
            return $this->client->createCredentials($result);
        })->otherwise(function (\RuntimeException $exception) {
            throw new \WP_Media_Folder\Aws\Exception\CredentialsException("Error in retrieving assume role credentials.", 0, $exception);
        });
    }
}
