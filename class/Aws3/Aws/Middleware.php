...
final class Middleware
{
     ...
     * @return callable
     */
    public static function sourceFile(\WP_Media_Folder\Aws\Api\Service $api, $bodyParameter = 'Body', $sourceParameter = 'SourceFile')
    {
        return function (callable $handler) use($api, $bodyParameter, $sourceParameter) {
            return function (\WP_Media_Folder\Aws\CommandInterface $command, \WP_Media_Folder\Psr\Http\Message\RequestInterface $request = null) use($handler, $api, $bodyParameter, $sourceParameter) {
                $operation = $api->getOperation($command->getName());
                $source = $command[$sourceParameter];
                if ($source !== null && $operation->getInput()->hasMember($bodyParameter)) {
                    $command[$bodyParameter] = new \WP_Media_Folder\GuzzleHttp\Psr7\LazyOpenStream($source, 'r');
     ...
     * @return callable
     */
    public static function validation(\WP_Media_Folder\Aws\Api\Service $api, \WP_Media_Folder\Aws\Api\Validator $validator = null)
    {
        $validator = $validator ?: new \WP_Media_Folder\Aws\Api\Validator();
        return function (callable $handler) use($api, $validator) {
            return function (\WP_Media_Folder\Aws\CommandInterface $command, \WP_Media_Folder\Psr\Http\Message\RequestInterface $request = null) use($api, $validator, $handler) {
                $operation = $api->getOperation($command->getName());
    ...
    public static function requestBuilder(callable $serializer)
    {
        return function (callable $handler) use($serializer) {
            return function (\WP_Media_Folder\Aws\CommandInterface $command) use($serializer, $handler) {
    ...
    public static function signer(callable $credProvider, callable $signatureFunction)
    {
        return function (callable $handler) use($signatureFunction, $credProvider) {
            return function (\WP_Media_Folder\Aws\CommandInterface $command, \WP_Media_Folder\Psr\Http\Message\RequestInterface $request) use($handler, $signatureFunction, $credProvider) {
                $signer = $signatureFunction($command);
                return $credProvider()->then(function (\WP_Media_Folder\Aws\Credentials\CredentialsInterface $creds) use($handler, $command, $signer, $request) {
    ...
    public static function tap(callable $fn)
    {
        return function (callable $handler) use($fn) {
            return function (\WP_Media_Folder\Aws\CommandInterface $command, \WP_Media_Folder\Psr\Http\Message\RequestInterface $request = null) use($handler, $fn) {
                $fn($command, $request);
                return $handler($command, $request);
            };
        };
    ...
    {
        $decider = $decider ?: \WP_Media_Folder\Aws\RetryMiddleware::createDefaultDecider();
        $delay = $delay ?: [\WP_Media_Folder\Aws\RetryMiddleware::class, 'exponentialDelay'];
        return function (callable $handler) use($decider, $delay, $stats) {
            return new \WP_Media_Folder\Aws\RetryMiddleware($decider, $delay, $handler, $stats);
        };
    }
    /**
    ...
    public static function invocationId()
    {
        return function (callable $handler) {
            return function (\WP_Media_Folder\Aws\CommandInterface $command, \WP_Media_Folder\Psr\Http\Message\RequestInterface $request) use($handler) {
    ...
    public static function contentType(array $operations)
    {
        return function (callable $handler) use($operations) {
            return function (\WP_Media_Folder\Aws\CommandInterface $command, \WP_Media_Folder\Psr\Http\Message\RequestInterface $request = null) use($handler, $operations) {
                if (!$request->hasHeader('Content-Type') && in_array($command->getName(), $operations, true) && ($uri = $request->getBody()->getMetadata('uri'))) {
                    $request = $request->withHeader('Content-Type', \WP_Media_Folder\GuzzleHttp\Psr7\mimetype_from_filename($uri) ?: 'application/octet-stream');
                }
                return $handler($command, $request);
            };
        };
     ...
     * @return callable
     */
    public static function history(\WP_Media_Folder\Aws\History $history)
    {
        return function (callable $handler) use($history) {
            return function (\WP_Media_Folder\Aws\CommandInterface $command, \WP_Media_Folder\Psr\Http\Message\RequestInterface $request = null) use($handler, $history) {
    ...
    public static function mapRequest(callable $f)
    {
        return function (callable $handler) use($f) {
            return function (\WP_Media_Folder\Aws\CommandInterface $command, \WP_Media_Folder\Psr\Http\Message\RequestInterface $request = null) use($handler, $f) {
    ...
    {
        return function (callable $handler) {
            return function (\WP_Media_Folder\Aws\CommandInterface $command, \WP_Media_Folder\Psr\Http\Message\RequestInterface $request = null) use($handler) {
                $start = microtime(true);
                return $handler($command, $request)->then(function (\WP_Media_Folder\Aws\ResultInterface $res) use($start) {
                    ...
                    return \WP_Media_Folder\GuzzleHttp\Promise\rejection_for($err);
                });
            };
        };
    }
