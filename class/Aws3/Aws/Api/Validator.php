...
class Validator
{
     ...
     */
    public function validate($name, \WP_Media_Folder\Aws\Api\Shape $shape, array $input)
    {
    ...
    private function dispatch(\WP_Media_Folder\Aws\Api\Shape $shape, $value)
    {
    ...
    private function check_structure(\WP_Media_Folder\Aws\Api\StructureShape $shape, $value)
    {
    ...
    private function check_list(\WP_Media_Folder\Aws\Api\ListShape $shape, $value)
    {
        if (!is_array($value)) {
            $this->addError('must be an array. Found ' . \WP_Media_Folder\Aws\describe_type($value));
            return;
        }
    ...
    private function check_map(\WP_Media_Folder\Aws\Api\MapShape $shape, $value)
    {
        if (!$this->checkAssociativeArray($value)) {
            return;
        }
    ...
    private function check_blob(\WP_Media_Folder\Aws\Api\Shape $shape, $value)
    {
        static $valid = ['string' => true, 'integer' => true, 'double' => true, 'resource' => true];
        $type = gettype($value);
        if (!isset($valid[$type])) {
            if ($type != 'object' || !method_exists($value, '__toString')) {
                $this->addError('must be an fopen resource, a ' . 'WP_Media_Folder\\GuzzleHttp\\Stream\\StreamInterface object, or something ' . 'that can be cast to a string. Found ' . \WP_Media_Folder\Aws\describe_type($value));
            }
        }
    }
    private function check_numeric(\WP_Media_Folder\Aws\Api\Shape $shape, $value)
    {
        if (!is_numeric($value)) {
            $this->addError('must be numeric. Found ' . \WP_Media_Folder\Aws\describe_type($value));
            return;
        }
        $this->validateRange($shape, $value, "numeric value");
    }
    private function check_boolean(\WP_Media_Folder\Aws\Api\Shape $shape, $value)
    {
        if (!is_bool($value)) {
            $this->addError('must be a boolean. Found ' . \WP_Media_Folder\Aws\describe_type($value));
        }
    }
    private function check_string(\WP_Media_Folder\Aws\Api\Shape $shape, $value)
    {
        if ($shape['jsonvalue']) {
            if (!self::canJsonEncode($value)) {
                $this->addError('must be a value encodable with \'json_encode\'.' . ' Found ' . \WP_Media_Folder\Aws\describe_type($value));
            }
            return;
        }
        if (!$this->checkCanString($value)) {
            $this->addError('must be a string or an object that implements ' . '__toString(). Found ' . \WP_Media_Folder\Aws\describe_type($value));
            return;
        }
    ...
    private function validateRange(\WP_Media_Folder\Aws\Api\Shape $shape, $length, $descriptor)
    {
    ...
    private function checkAssociativeArray($value)
    {
        if (!is_array($value) || isset($value[0])) {
            $this->addError('must be an associative array. Found ' . \WP_Media_Folder\Aws\describe_type($value));
            return false;
        }
        return true;
    }
