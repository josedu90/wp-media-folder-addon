...
trait PayloadParserTrait
{
    ...
    private function parseJson($json, $response)
    {
        ...
        if (JSON_ERROR_NONE !== json_last_error()) {
            throw new \WP_Media_Folder\Aws\Api\Parser\Exception\ParserException('Error parsing JSON: ' . json_last_error_msg(), 0, null, ['response' => $response]);
        }
        return $jsonPayload;
    }
    /**
    ...
    private function parseXml($xml, $response)
    {
        ...
        try {
            ...
            throw new \WP_Media_Folder\Aws\Api\Parser\Exception\ParserException("Error parsing XML: {$e->getMessage()}", 0, $e, ['response' => $response]);
        } finally {
