...
class DecodingEventStreamIterator implements \Iterator
{
    ...
    private function parseHeaders($headerBytes)
    {
        ...
        while ($bytesRead < $headerBytes) {
            ...
            if (isset($headers[$key])) {
                throw new \WP_Media_Folder\Aws\Api\Parser\Exception\ParserException('Duplicate key in event headers.');
            }
            $headers[$key] = $value;
        }
    ...
    private function parsePrelude()
    {
        ...
        if (unpack('N', $calculatedCrc)[1] !== $prelude[self::CRC_PRELUDE]) {
            throw new \WP_Media_Folder\Aws\Api\Parser\Exception\ParserException('Prelude checksum mismatch.');
        }
    ...
    private function parseEvent()
    {
        ...
        if ($this->stream->tell() < $this->stream->getSize()) {
            ...
            if ($prelude[self::LENGTH_TOTAL] > $bytesLeft) {
                throw new \WP_Media_Folder\Aws\Api\Parser\Exception\ParserException('Message length too long.');
            }
            $bytesLeft -= $numBytes;
            if ($prelude[self::LENGTH_HEADERS] > $bytesLeft) {
                throw new \WP_Media_Folder\Aws\Api\Parser\Exception\ParserException('Headers length too long.');
            }
            list($event[self::HEADERS], $numBytes) = $this->parseHeaders($prelude[self::LENGTH_HEADERS]);
            $event[self::PAYLOAD] = \WP_Media_Folder\GuzzleHttp\Psr7\stream_for($this->readAndHashBytes($prelude[self::LENGTH_TOTAL] - self::BYTES_PRELUDE - $numBytes - self::BYTES_TRAILING));
            $calculatedCrc = hash_final($this->hashContext, true);
            $messageCrc = $this->stream->read(4);
            if ($calculatedCrc !== $messageCrc) {
                throw new \WP_Media_Folder\Aws\Api\Parser\Exception\ParserException('Message checksum mismatch.');
            }
        }
        return $event;
    }
    ...
    private function decodeBytes($lengthBytes = 2)
    {
        if (!isset(self::$lengthFormatMap[$lengthBytes])) {
            throw new \WP_Media_Folder\Aws\Api\Parser\Exception\ParserException('Undefined variable length format.');
        }
    ...
    private function decodeTimestamp()
    {
        ...
        return [\WP_Media_Folder\Aws\Api\DateTimeResult::createFromFormat('U.u', $val / 1000), $bytes];
    }
