...
class WrappedHttpHandler
{
     ...
     *                                 information.
     */
    public function __construct(callable $httpHandler, callable $parser, callable $errorParser, $exceptionClass = 'WP_Media_Folder\\Aws\\Exception\\AwsException', $collectStats = false)
    {
    ...
    public function __invoke(\WP_Media_Folder\Aws\CommandInterface $command, \WP_Media_Folder\Psr\Http\Message\RequestInterface $request)
    {
        ...
        return \WP_Media_Folder\GuzzleHttp\Promise\promise_for($fn($request, $options))->then(function (\WP_Media_Folder\Psr\Http\Message\ResponseInterface $res) use($command, $request, &$stats) {
            ...
            return new \WP_Media_Folder\GuzzleHttp\Promise\RejectedPromise($err);
        });
    }
    /**
     ...
     */
    private function parseResponse(\WP_Media_Folder\Aws\CommandInterface $command, \WP_Media_Folder\Psr\Http\Message\RequestInterface $request, \WP_Media_Folder\Psr\Http\Message\ResponseInterface $response, array $stats)
    {
        $parser = $this->parser;
        $status = $response->getStatusCode();
        $result = $status < 300 ? $parser($command, $response) : new \WP_Media_Folder\Aws\Result();
     ...
     * @return \Exception
     */
    private function parseError(array $err, \WP_Media_Folder\Psr\Http\Message\RequestInterface $request, \WP_Media_Folder\Aws\CommandInterface $command, array $stats)
    {
