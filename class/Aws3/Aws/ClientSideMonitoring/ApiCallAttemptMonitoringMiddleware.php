 ...
 * @internal
 */
class ApiCallAttemptMonitoringMiddleware extends \WP_Media_Folder\Aws\ClientSideMonitoring\AbstractMonitoringMiddleware
{
    /**
     ...
     * {@inheritdoc}
     */
    public static function getRequestData(\WP_Media_Folder\Psr\Http\Message\RequestInterface $request)
    {
    ...
    private static function getResultAttemptLatency(\WP_Media_Folder\Aws\ResultInterface $result)
    {
    ...
    private static function getResultDestinationIp(\WP_Media_Folder\Aws\ResultInterface $result)
    {
        if (isset($result['@metadata']['transferStats']['http'])) {
            $attempt = end($result['@metadata']['transferStats']['http']);
    ...
    private static function getResultDnsLatency(\WP_Media_Folder\Aws\ResultInterface $result)
    {
        if (isset($result['@metadata']['transferStats']['http'])) {
            $attempt = end($result['@metadata']['transferStats']['http']);
    ...
    private static function getResultHttpStatusCode(\WP_Media_Folder\Aws\ResultInterface $result)
    {
        return $result['@metadata']['statusCode'];
    }
    private static function getAwsExceptionAttemptLatency(\WP_Media_Folder\Aws\Exception\AwsException $e)
    {
    ...
    private static function getAwsExceptionErrorCode(\WP_Media_Folder\Aws\Exception\AwsException $e)
    {
        return $e->getAwsErrorCode();
    }
    private static function getAwsExceptionMessage(\WP_Media_Folder\Aws\Exception\AwsException $e)
    {
        return $e->getAwsErrorMessage();
    }
    private static function getAwsExceptionDestinationIp(\WP_Media_Folder\Aws\Exception\AwsException $e)
    {
        $attempt = $e->getTransferInfo();
        if (isset($attempt['primary_ip'])) {
            return $attempt['primary_ip'];
    ...
    private static function getAwsExceptionDnsLatency(\WP_Media_Folder\Aws\Exception\AwsException $e)
    {
        $attempt = $e->getTransferInfo();
        if (isset($attempt['namelookup_time'])) {
            return (int) floor($attempt['namelookup_time'] * 1000);
    ...
    private static function getAwsExceptionHttpStatusCode(\WP_Media_Folder\Aws\Exception\AwsException $e)
    {
     ...
     * {@inheritdoc}
     */
    protected function populateRequestEventData(\WP_Media_Folder\Aws\CommandInterface $cmd, \WP_Media_Folder\Psr\Http\Message\RequestInterface $request, array $event)
    {
