...
use WP_Media_Folder\Aws\ClientSideMonitoring\Exception\ConfigurationException;
...
class ConfigurationProvider
{
    ...
    public static function cache(callable $provider, \WP_Media_Folder\Aws\CacheInterface $cache, $cacheKey = null)
    {
        ...
        return function () use($provider, $cache, $cacheKey) {
            ...
            return $provider()->then(function (\WP_Media_Folder\Aws\ClientSideMonitoring\ConfigurationInterface $config) use($cache, $cacheKey) {
    ...
    public static function env()
    {
        return function () {
            ...
            if ($enabled !== false) {
                return \WP_Media_Folder\GuzzleHttp\Promise\promise_for(new \WP_Media_Folder\Aws\ClientSideMonitoring\Configuration($enabled, getenv(self::ENV_PORT) ?: self::DEFAULT_PORT, getenv(self::ENV_CLIENT_ID) ?: self::DEFAULT_CLIENT_ID));
            }
    ...
    {
        return function () {
            return \WP_Media_Folder\GuzzleHttp\Promise\promise_for(new \WP_Media_Folder\Aws\ClientSideMonitoring\Configuration(self::DEFAULT_ENABLED, self::DEFAULT_PORT, self::DEFAULT_CLIENT_ID));
        };
    }
    /**
    ...
    public static function ini($profile = null, $filename = null)
    {
        ...
        return function () use($profile, $filename) {
            ...
            return \WP_Media_Folder\GuzzleHttp\Promise\promise_for(new \WP_Media_Folder\Aws\ClientSideMonitoring\Configuration($data[$profile]['csm_enabled'], $data[$profile]['csm_port'], $data[$profile]['csm_client_id']));
        };
    }
    /**
    ...
    public static function memoize(callable $provider)
    {
        return function () use($provider) {
            ...
            return $result->then(function (\WP_Media_Folder\Aws\ClientSideMonitoring\ConfigurationInterface $config) use(&$isConstant) {
                $isConstant = true;
                return $config;
            });
        };
    ...
    {
        return new \WP_Media_Folder\GuzzleHttp\Promise\RejectedPromise(new \WP_Media_Folder\Aws\ClientSideMonitoring\Exception\ConfigurationException($msg));
    }
    /**
    ...
    public static function unwrap($config)
    {
        ...
        if ($config instanceof ConfigurationInterface) {
            ...
            return new \WP_Media_Folder\Aws\ClientSideMonitoring\Configuration($config['enabled'], $port, $client_id);
        }
