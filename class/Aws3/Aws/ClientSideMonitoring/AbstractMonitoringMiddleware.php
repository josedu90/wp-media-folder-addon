 ...
 * @internal
 */
abstract class AbstractMonitoringMiddleware implements \WP_Media_Folder\Aws\ClientSideMonitoring\MonitoringMiddlewareInterface
{
    ...
    protected $region;
    protected $service;
    protected static function getAwsExceptionHeader(\WP_Media_Folder\Aws\Exception\AwsException $e, $headerName)
    {
    ...
    protected static function getResultHeader(\WP_Media_Folder\Aws\ResultInterface $result, $headerName)
    {
     ...
     */
    public function __invoke(\WP_Media_Folder\Aws\CommandInterface $cmd, \WP_Media_Folder\Psr\Http\Message\RequestInterface $request)
    {
        ...
        $g = function ($value) use($eventData, $enabled) {
            ...
            if ($value instanceof \Exception || $value instanceof \Throwable) {
                return \WP_Media_Folder\GuzzleHttp\Promise\rejection_for($value);
            }
            return $value;
        };
        return \WP_Media_Folder\GuzzleHttp\Promise\promise_for($handler($cmd, $request))->then($g, $g);
    }
    private function getClientId()
    {
        return $this->unwrappedOptions()->getClientId();
    }
    private function getNewEvent(\WP_Media_Folder\Aws\CommandInterface $cmd, \WP_Media_Folder\Psr\Http\Message\RequestInterface $request)
    {
        $event = ['Api' => $cmd->getName(), 'ClientId' => $this->getClientId(), 'Region' => $this->getRegion(), 'Service' => $this->getService(), 'Timestamp' => (int) floor(microtime(true) * 1000), 'UserAgent' => substr($request->getHeaderLine('User-Agent') . ' ' . \WP_Media_Folder\Aws\default_user_agent(), 0, 256), 'Version' => 1];
        return $event;
    }
    ...
    private function unwrappedOptions()
    {
        if (!$this->options instanceof ConfigurationInterface) {
            $this->options = \WP_Media_Folder\Aws\ClientSideMonitoring\ConfigurationProvider::unwrap($this->options);
        }
        return $this->options;
    }
}
