...
use WP_Media_Folder\GuzzleHttp\Event\EndEvent;
use WP_Media_Folder\GuzzleHttp\Exception\ConnectException;
use WP_Media_Folder\GuzzleHttp\Exception\RequestException;
use WP_Media_Folder\GuzzleHttp\Message\ResponseInterface as GuzzleResponse;
use WP_Media_Folder\GuzzleHttp\Promise;
use WP_Media_Folder\GuzzleHttp\Psr7\Response as Psr7Response;
use WP_Media_Folder\GuzzleHttp\Stream\Stream;
...
class GuzzleHandler
{
    ...
    public function __invoke(\WP_Media_Folder\Psr\Http\Message\RequestInterface $request, array $options = [])
    {
        ...
        $promise = new \WP_Media_Folder\GuzzleHttp\Promise\Promise(function () use($guzzlePromise) {
            try {
                $guzzlePromise->wait();
            } catch (\Exception $e) {
        ...
        return $promise->then(function (\WP_Media_Folder\GuzzleHttp\Message\ResponseInterface $response) {
            ...
            if ($exception instanceof RequestException) {
                if (isset($options['sink'])) {
                    if (!$options['sink'] instanceof Psr7StreamInterface) {
                        $exception->getResponse()->setBody(\WP_Media_Folder\GuzzleHttp\Stream\Stream::factory(file_get_contents($options['sink'])));
                    }
                }
            }
            // Reject with information about the error.
            return new \WP_Media_Folder\GuzzleHttp\Promise\RejectedPromise($this->prepareErrorData($exception));
        });
    }
    private function createGuzzleRequest(\WP_Media_Folder\Psr\Http\Message\RequestInterface $psrRequest, array $options)
    {
        $ringConfig = [];
        ...
        // Prepare sink option.
        if (isset($options['sink'])) {
            $ringConfig['save_to'] = $options['sink'] instanceof Psr7StreamInterface ? new \WP_Media_Folder\Aws\Handler\GuzzleV5\GuzzleStream($options['sink']) : $options['sink'];
            unset($options['sink']);
        }
        ...
        if (is_callable($statsCallback)) {
            $request->getEmitter()->on('end', function (\WP_Media_Folder\GuzzleHttp\Event\EndEvent $event) use($statsCallback) {
        ...
        if ($body->getSize() === 0) {
            ...
            $request->setBody(new \WP_Media_Folder\Aws\Handler\GuzzleV5\GuzzleStream($body));
        }
        $request->setHeaders($psrRequest->getHeaders());
        $request->setHeader('User-Agent', $request->getHeader('User-Agent') . ' ' . \WP_Media_Folder\GuzzleHttp\Client::getDefaultUserAgent());
    ...
    private function createPsr7Response(\WP_Media_Folder\GuzzleHttp\Message\ResponseInterface $response)
    {
        if ($body = $response->getBody()) {
            $body = new \WP_Media_Folder\Aws\Handler\GuzzleV5\PsrStream($body);
        }
        return new \WP_Media_Folder\GuzzleHttp\Psr7\Response($response->getStatusCode(), $response->getHeaders(), $body, $response->getReasonPhrase());
    }
