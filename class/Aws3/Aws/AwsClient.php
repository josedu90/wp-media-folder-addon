...
use WP_Media_Folder\Aws\ClientSideMonitoring\ApiCallAttemptMonitoringMiddleware;
use WP_Media_Folder\Aws\ClientSideMonitoring\ApiCallMonitoringMiddleware;
use WP_Media_Folder\Aws\ClientSideMonitoring\ConfigurationProvider;
use WP_Media_Folder\Aws\Signature\SignatureProvider;
use WP_Media_Folder\GuzzleHttp\Psr7\Uri;
/**
 * Default AWS client implementation
 */
class AwsClient implements \WP_Media_Folder\Aws\AwsClientInterface
{
    use AwsClientTrait;
    /** @var array */
    private $config;
    ...
    {
        return \WP_Media_Folder\Aws\ClientResolver::getDefaultArguments();
    }
    /**
    ...
    public function __construct(array $args)
    {
        ...
        $this->handlerList = new \WP_Media_Folder\Aws\HandlerList();
        $resolver = new \WP_Media_Folder\Aws\ClientResolver(static::getArguments());
        $config = $resolver->resolve($args, $this->handlerList);
        $this->api = $config['api'];
        $this->signatureProvider = $config['signature_provider'];
        $this->endpoint = new \WP_Media_Folder\GuzzleHttp\Psr7\Uri($config['endpoint']);
    ...
    public function getCommand($name, array $args = [])
    {
        ...
        return new \WP_Media_Folder\Aws\Command($name, $args, clone $this->getHandlerList());
    }
    ...
    private function parseClass()
    {
        ...
        if ($klass === __CLASS__) {
            return ['', 'WP_Media_Folder\\Aws\\Exception\\AwsException'];
        }
        $service = substr($klass, strrpos($klass, '\\') + 1, -6);
        return [strtolower($service), "WP_Media_Folder\\Aws\\{$service}\\Exception\\{$service}Exception"];
    }
    private function addEndpointParameterMiddleware($args)
    {
        if (empty($args['disable_host_prefix_injection'])) {
            $list = $this->getHandlerList();
            $list->appendBuild(\WP_Media_Folder\Aws\EndpointParameterMiddleware::wrap($this->api), 'endpoint_parameter');
        }
    }
    private function addSignatureMiddleware()
    {
        ...
        $resolver = static function (\WP_Media_Folder\Aws\CommandInterface $c) use($api, $provider, $name, $region, $version) {
            ...
            return \WP_Media_Folder\Aws\Signature\SignatureProvider::resolve($provider, $version, $name, $region);
        };
        $this->handlerList->appendSign(\WP_Media_Folder\Aws\Middleware::signer($this->credentialProvider, $resolver), 'signer');
    }
    private function addInvocationId()
    {
        // Add invocation id to each request
        $this->handlerList->prependSign(\WP_Media_Folder\Aws\Middleware::invocationId(), 'invocation-id');
    }
    private function addClientSideMonitoring($args)
    {
        $options = \WP_Media_Folder\Aws\ClientSideMonitoring\ConfigurationProvider::defaultProvider($args);
        $this->handlerList->appendBuild(\WP_Media_Folder\Aws\ClientSideMonitoring\ApiCallMonitoringMiddleware::wrap($this->credentialProvider, $options, $this->region, $this->getApi()->getServiceId()), 'ApiCallMonitoringMiddleware');
        $callAttemptMiddleware = \WP_Media_Folder\Aws\ClientSideMonitoring\ApiCallAttemptMonitoringMiddleware::wrap($this->credentialProvider, $options, $this->region, $this->getApi()->getServiceId());
