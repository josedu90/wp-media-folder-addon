...
use WP_Media_Folder\Aws\S3\MultipartUploader;
use WP_Media_Folder\Aws\S3\S3ClientInterface;
use WP_Media_Folder\GuzzleHttp\Promise;
/**
 * Encapsulates the execution of a multipart upload of an encrypted object to S3.
 */
class S3EncryptionMultipartUploader extends \WP_Media_Folder\Aws\S3\MultipartUploader
{
    ...
    private function getEncryptingDataPreparer()
    {
        return function () {
            ...
            list($this->source, $params) = \WP_Media_Folder\GuzzleHttp\Promise\promise_for($this->encrypt($this->source, $this->config['@cipheroptions'] ?: [], $this->provider, $envelope))->then(function ($bodyStream) use($envelope) {
