...
use WP_Media_Folder\Aws\AwsClientInterface;
use WP_Media_Folder\Aws\S3\Exception\DeleteMultipleObjectsException;
 ...
 */
class BatchDelete implements \WP_Media_Folder\GuzzleHttp\Promise\PromisorInterface
{
    private $bucket;
     ...
     * @return BatchDelete
     */
    public static function fromListObjects(\WP_Media_Folder\Aws\AwsClientInterface $client, array $listObjectsParams, array $options = [])
    {
        $iter = $client->getPaginator('ListObjects', $listObjectsParams);
        $bucket = $listObjectsParams['Bucket'];
        $fn = function (\WP_Media_Folder\Aws\S3\BatchDelete $that) use($iter) {
            return $iter->each(function ($result) use($that) {
                ...
                return $promises ? \WP_Media_Folder\GuzzleHttp\Promise\all($promises) : null;
            });
        };
     ...
     * @return BatchDelete
     */
    public static function fromIterator(\WP_Media_Folder\Aws\AwsClientInterface $client, $bucket, \Iterator $iter, array $options = [])
    {
        $fn = function (\WP_Media_Folder\Aws\S3\BatchDelete $that) use($iter) {
            return \WP_Media_Folder\GuzzleHttp\Promise\coroutine(function () use($that, $iter) {
                foreach ($iter as $obj) {
     ...
     */
    private function __construct(\WP_Media_Folder\Aws\AwsClientInterface $client, $bucket, callable $promiseFn, array $options = [])
    {
        $this->client = $client;
        $this->bucket = $bucket;
    ...
    private function flushQueue()
    {
        ...
        return $this->client->executeAsync($command)->then(function ($result) {
            if (!empty($result['Errors'])) {
                throw new \WP_Media_Folder\Aws\S3\Exception\DeleteMultipleObjectsException($result['Deleted'] ?: [], $result['Errors']);
            }
            return $result;
        });
    }
    ...
    private function createPromise()
    {
        ...
        return $promise->then(function () use($cleanup) {
            return \WP_Media_Folder\GuzzleHttp\Promise\promise_for($this->flushQueue())->then($cleanup);
