...
class S3UriParser
{
    ...
    {
        $url = \WP_Media_Folder\GuzzleHttp\Psr7\uri_for($uri);
    ...
    private function parseStreamWrapper(\WP_Media_Folder\Psr\Http\Message\UriInterface $url)
    {
    ...
    private function parseCustomEndpoint(\WP_Media_Folder\Psr\Http\Message\UriInterface $url)
    {
        $result = self::$defaultResult;
    ...
    private function parsePathStyle(\WP_Media_Folder\Psr\Http\Message\UriInterface $url)
    {
        $result = self::$defaultResult;
        if ($url->getPath() != '/') {
            $path = ltrim($url->getPath(), '/');
    ...
    private function parseVirtualHosted(\WP_Media_Folder\Psr\Http\Message\UriInterface $url, array $matches)
    {
        $result = self::$defaultResult;
        $result['path_style'] = false;
