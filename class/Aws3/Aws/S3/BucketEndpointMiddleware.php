...
class BucketEndpointMiddleware
{
    ...
    private function modifyRequest(\WP_Media_Folder\Psr\Http\Message\RequestInterface $request, \WP_Media_Folder\Aws\CommandInterface $command)
    {
        $uri = $request->getUri();
        $path = $uri->getPath();
        $bucket = $command['Bucket'];
        ...
        if ($command['Key']) {
            $path = \WP_Media_Folder\Aws\S3\S3Client::encodeKey(rawurldecode($path));
        }
