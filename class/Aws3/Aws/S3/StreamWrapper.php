...
use WP_Media_Folder\GuzzleHttp\Psr7\CachingStream;
...
class StreamWrapper
{
     ...
     */
    public static function register(\WP_Media_Folder\Aws\S3\S3ClientInterface $client, $protocol = 's3', \WP_Media_Folder\Aws\CacheInterface $cache = null)
    {
        ...
        if ($cache) {
            ...
            $default[$protocol]['cache'] = new \WP_Media_Folder\Aws\LruArrayCache();
        }
    ...
    public function stream_flush()
    {
        ...
        if (!isset($params['ContentType']) && ($type = \WP_Media_Folder\GuzzleHttp\Psr7\mimetype_from_filename($params['Key']))) {
    ...
    public function dir_opendir($path, $options)
    {
        ...
        $this->objectIterator = \WP_Media_Folder\Aws\flatmap($this->getClient()->getPaginator('ListObjects', $op), function (\WP_Media_Folder\Aws\Result $result) use($filterFn) {
    ...
    private function openReadStream()
    {
        ...
        if ($this->getOption('seekable') && !$this->body->isSeekable()) {
            $this->body = new \WP_Media_Folder\GuzzleHttp\Psr7\CachingStream($this->body);
        }
        return true;
    }
    private function openWriteStream()
    {
        $this->body = new \WP_Media_Folder\GuzzleHttp\Psr7\Stream(fopen('php://temp', 'r+'));
        return true;
    }
    ...
    {
        if (!$this->cache) {
            $this->cache = $this->getOption('cache') ?: new \WP_Media_Folder\Aws\LruArrayCache();
        }
        return $this->cache;
    }
    /**
