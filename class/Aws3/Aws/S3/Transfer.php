...
use WP_Media_Folder\GuzzleHttp\Promise\PromisorInterface;
use Iterator;
/**
 ...
 */
class Transfer implements \WP_Media_Folder\GuzzleHttp\Promise\PromisorInterface
{
    private $client;
    private $promise;
    private $source;
     ...
     */
    public function __construct(\WP_Media_Folder\Aws\S3\S3ClientInterface $client, $source, $dest, array $options = [])
    {
        $this->client = $client;
        ...
        $this->concurrency = isset($options['concurrency']) ? $options['concurrency'] : \WP_Media_Folder\Aws\S3\MultipartUploader::DEFAULT_CONCURRENCY;
        $this->mupThreshold = isset($options['mup_threshold']) ? $options['mup_threshold'] : 16777216;
        if ($this->mupThreshold < \WP_Media_Folder\Aws\S3\MultipartUploader::PART_MIN_SIZE) {
    ...
    private function createDownloadPromise()
    {
        ...
        foreach ($this->getDownloadsIterator() as $object) {
            ...
            if (strpos($this->resolveUri($resolveSink), $this->destination['path']) !== 0) {
                throw new \WP_Media_Folder\Aws\Exception\AwsException('Cannot download key ' . $objectKey . ', its relative path resolves outside the' . ' parent directory', $command);
            }
        ...
        return (new \WP_Media_Folder\Aws\CommandPool($this->client, $commands, ['concurrency' => $this->concurrency, 'before' => $this->before, 'rejected' => function ($reason, $idx, \WP_Media_Folder\GuzzleHttp\Promise\PromiseInterface $p) {
            $p->reject($reason);
        }]))->promise();
    }
    private function createUploadPromise()
    {
        ...
        $files = \WP_Media_Folder\Aws\map($this->getUploadsIterator(), function ($file) {
        ...
        return \WP_Media_Folder\GuzzleHttp\Promise\each_limit_all($files, $this->concurrency);
    }
    /** @return Iterator */
    private function getUploadsIterator()
    {
        if (is_string($this->source)) {
            return \WP_Media_Folder\Aws\filter(\WP_Media_Folder\Aws\recursive_dir_iterator($this->sourceMetadata['path']), function ($file) {
                return !is_dir($file);
            });
        }
        return $this->source;
    ...
    private function getDownloadsIterator()
    {
        if (is_string($this->source)) {
            ...
            $files = \WP_Media_Folder\Aws\map($files, function ($key) use($listArgs) {
                return "s3://{$listArgs['Bucket']}/{$key}";
            });
            return \WP_Media_Folder\Aws\filter($files, function ($key) {
    ...
    private function uploadMultipart($filename)
    {
        $args = $this->s3Args;
        $args['Key'] = $this->createS3Key($filename);
        return (new \WP_Media_Folder\Aws\S3\MultipartUploader($this->client, $filename, ['bucket' => $args['Bucket'], 'key' => $args['Key'], 'before_initiate' => $this->before, 'before_upload' => $this->before, 'before_complete' => $this->before, 'concurrency' => $this->concurrency]))->promise();
    }
    ...
    private function addDebugToBefore($debug)
    {
        ...
        $s3Args = $this->s3Args;
        $this->before = static function (\WP_Media_Folder\Aws\CommandInterface $command) use($before, $debug, $sourcePath, $s3Args) {
