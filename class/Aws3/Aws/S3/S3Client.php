 ...
 */
class S3Client extends \WP_Media_Folder\Aws\AwsClient implements \WP_Media_Folder\Aws\S3\S3ClientInterface
{
    use S3ClientTrait;
    ...
    public function __construct(array $args)
    {
        ...
        $stack->appendInit(\WP_Media_Folder\Aws\S3\SSECMiddleware::wrap($this->getEndpoint()->getScheme()), 's3.ssec');
        $stack->appendBuild(\WP_Media_Folder\Aws\S3\ApplyChecksumMiddleware::wrap(), 's3.checksum');
        $stack->appendBuild(\WP_Media_Folder\Aws\Middleware::contentType(['PutObject', 'UploadPart']), 's3.content_type');
        // Use the bucket style middleware when using a "bucket_endpoint" (for cnames)
        if ($this->getConfig('bucket_endpoint')) {
            $stack->appendBuild(\WP_Media_Folder\Aws\S3\BucketEndpointMiddleware::wrap(), 's3.bucket_endpoint');
        } else {
            $stack->appendBuild(\WP_Media_Folder\Aws\S3\S3EndpointMiddleware::wrap($this->getRegion(), ['dual_stack' => $this->getConfig('use_dual_stack_endpoint'), 'accelerate' => $this->getConfig('use_accelerate_endpoint'), 'path_style' => $this->getConfig('use_path_style_endpoint')]), 's3.endpoint_middleware');
        }
        $stack->appendSign(\WP_Media_Folder\Aws\S3\PutObjectUrlMiddleware::wrap(), 's3.put_object_url');
        $stack->appendSign(\WP_Media_Folder\Aws\S3\PermanentRedirectMiddleware::wrap(), 's3.permanent_redirect');
        $stack->appendInit(\WP_Media_Folder\Aws\Middleware::sourceFile($this->getApi()), 's3.source_file');
    ...
    public function createPresignedRequest(\WP_Media_Folder\Aws\CommandInterface $command, $expires)
    {
        ...
        return $signer->presign(\WP_Media_Folder\Aws\serialize($command), $this->getCredentials()->wait(), $expires);
    }
    ...
    private function getLocationConstraintMiddleware()
    {
        ...
        return static function (callable $handler) use($region) {
            return function (\WP_Media_Folder\Aws\Command $command, $request = null) use($handler, $region) {
    ...
    private function getSaveAsParameter()
    {
        return static function (callable $handler) {
            return function (\WP_Media_Folder\Aws\Command $command, $request = null) use($handler) {
    ...
    private function getEncodingTypeMiddleware()
    {
        return static function (callable $handler) {
            return function (\WP_Media_Folder\Aws\Command $command, $request = null) use($handler) {
                ...
                return $handler($command, $request)->then(function (\WP_Media_Folder\Aws\ResultInterface $result) use($autoSet) {
    ...
    /** @internal */
    public static function _applyRetryConfig($value, $_, \WP_Media_Folder\Aws\HandlerList $list)
    {
        if (!$value) {
            return;
        }
        ...
        $delay = [\WP_Media_Folder\Aws\RetryMiddleware::class, 'exponentialDelay'];
        $list->appendSign(\WP_Media_Folder\Aws\Middleware::retry($decider, $delay), 'retry');
    }
    /** @internal */
    public static function _applyApiProvider($value, array &$args, \WP_Media_Folder\Aws\HandlerList $list)
    {
        \WP_Media_Folder\Aws\ClientResolver::_apply_api_provider($value, $args, $list);
        $args['parser'] = new \WP_Media_Folder\Aws\S3\GetBucketLocationParser(new \WP_Media_Folder\Aws\S3\AmbiguousSuccessParser(new \WP_Media_Folder\Aws\S3\RetryableMalformedResponseParser($args['parser'], $args['exception_class']), $args['error_parser'], $args['exception_class']));
    }
    /**
     * @internal
     * @codeCoverageIgnore
    ...
    public static function applyDocFilters(array $api, array $docs)
    {
        ...
        return [new \WP_Media_Folder\Aws\Api\Service($api, \WP_Media_Folder\Aws\Api\ApiProvider::defaultProvider()), new \WP_Media_Folder\Aws\Api\DocModel($docs)];
    }
}
