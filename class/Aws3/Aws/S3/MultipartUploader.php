...
use WP_Media_Folder\Aws\Multipart\AbstractUploader;
use WP_Media_Folder\Aws\PhpHash;
use WP_Media_Folder\Aws\ResultInterface;
use WP_Media_Folder\GuzzleHttp\Psr7;
use WP_Media_Folder\Psr\Http\Message\StreamInterface as Stream;
use WP_Media_Folder\Aws\S3\Exception\S3MultipartUploadException;
/**
 * Encapsulates the execution of a multipart upload to S3 or Glacier.
 */
class MultipartUploader extends \WP_Media_Folder\Aws\Multipart\AbstractUploader
{
     ...
     */
    public function __construct(\WP_Media_Folder\Aws\S3\S3ClientInterface $client, $source, array $config = [])
    {
        parent::__construct($client, $source, array_change_key_case($config) + ['bucket' => null, 'key' => null, 'exception_class' => \WP_Media_Folder\Aws\S3\Exception\S3MultipartUploadException::class]);
    }
    ...
    protected function createPart($seekable, $number)
    {
        ...
        if ($seekable) {
            ...
            $body = $this->limitPartStream(new \WP_Media_Folder\GuzzleHttp\Psr7\LazyOpenStream($this->source->getMetadata('uri'), 'r'));
        } else {
            ...
            $body = \WP_Media_Folder\GuzzleHttp\Psr7\stream_for();
            \WP_Media_Folder\GuzzleHttp\Psr7\copy_to_stream($source, $body);
        }
    ...
    protected function extractETag(\WP_Media_Folder\Aws\ResultInterface $result)
    {
        return $result['ETag'];
    }
    protected function getSourceMimeType()
    {
        if ($uri = $this->source->getMetadata('uri')) {
            return \WP_Media_Folder\GuzzleHttp\Psr7\mimetype_from_filename($uri) ?: 'application/octet-stream';
        }
    }
     ...
     * @return Stream
     */
    private function decorateWithHashes(\WP_Media_Folder\Psr\Http\Message\StreamInterface $stream, array &$data)
    {
        // Decorate source with a hashing stream
        $hash = new \WP_Media_Folder\Aws\PhpHash('sha256');
        return new \WP_Media_Folder\Aws\HashingStream($stream, $hash, function ($result) use(&$data) {
