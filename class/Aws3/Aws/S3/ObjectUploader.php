 ...
 * appropriate.
 */
class ObjectUploader implements \WP_Media_Folder\GuzzleHttp\Promise\PromisorInterface
{
     ...
     *                                          the sub command(s).
     */
    public function __construct(\WP_Media_Folder\Aws\S3\S3ClientInterface $client, $bucket, $key, $body, $acl = 'private', array $options = [])
    {
        $this->client = $client;
        $this->bucket = $bucket;
        $this->key = $key;
        $this->body = \WP_Media_Folder\GuzzleHttp\Psr7\stream_for($body);
        $this->acl = $acl;
    ...
    public function promise()
    {
        ...
        if ($this->requiresMultipart($this->body, $mup_threshold)) {
            ...
            return (new \WP_Media_Folder\Aws\S3\MultipartUploader($this->client, $this->body, ['bucket' => $this->bucket, 'key' => $this->key, 'acl' => $this->acl] + $this->options))->promise();
        }
     ...
     * @return bool
     */
    private function requiresMultipart(\WP_Media_Folder\Psr\Http\Message\StreamInterface &$body, $threshold)
    {
         ...
         */
        $buffer = \WP_Media_Folder\GuzzleHttp\Psr7\stream_for();
        \WP_Media_Folder\GuzzleHttp\Psr7\copy_to_stream($body, $buffer, \WP_Media_Folder\Aws\S3\MultipartUploader::PART_MIN_SIZE);
        // If body < 5MB, use PutObject with the buffer.
        if ($buffer->getSize() < \WP_Media_Folder\Aws\S3\MultipartUploader::PART_MIN_SIZE) {
            $buffer->seek(0);
            $body = $buffer;
            return false;
        }
        ...
        if ($body->isSeekable()) {
            ...
            // original stream.
            $buffer->seek(0);
            $body = new \WP_Media_Folder\GuzzleHttp\Psr7\AppendStream([$buffer, $body]);
        }
        return true;
    }
}
