 ...
 * when appropriate.
 */
class ObjectCopier implements \WP_Media_Folder\GuzzleHttp\Promise\PromisorInterface
{
    const DEFAULT_MULTIPART_THRESHOLD = \WP_Media_Folder\Aws\S3\MultipartUploader::PART_MAX_SIZE;
    private $client;
    private $source;
    private $destination;
    private $acl;
     ...
     */
    public function __construct(\WP_Media_Folder\Aws\S3\S3ClientInterface $client, array $source, array $destination, $acl = 'private', array $options = [])
    {
    ...
    public function promise()
    {
        return \WP_Media_Folder\GuzzleHttp\Promise\coroutine(function () {
            ...
            if ($objectStats['ContentLength'] > $this->options['mup_threshold']) {
                $mup = new \WP_Media_Folder\Aws\S3\MultipartCopy($this->client, $this->getSourcePath(), ['source_metadata' => $objectStats, 'acl' => $this->acl] + $this->destination + $this->options);
                (yield $mup->promise());
            } else {
