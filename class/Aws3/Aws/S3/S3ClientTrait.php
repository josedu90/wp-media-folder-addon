...
trait S3ClientTrait
{
    ...
    {
        return (new \WP_Media_Folder\Aws\S3\ObjectUploader($this, $bucket, $key, $body, $acl, $options))->promise();
    }
    /**
    ...
    public function copyAsync($fromB, $fromK, $destB, $destK, $acl = 'private', array $opts = [])
    {
        ...
        return (new \WP_Media_Folder\Aws\S3\ObjectCopier($this, $source, $destination, $acl, $opts))->promise();
    }
    /**
    ...
    {
        \WP_Media_Folder\Aws\S3\StreamWrapper::register($this);
    }
    /**
    ...
    {
        if (!$prefix && !$regex) {
            return new \WP_Media_Folder\GuzzleHttp\Promise\RejectedPromise(new \RuntimeException('A prefix or regex is required.'));
        }
        $params = ['Bucket' => $bucket, 'Prefix' => $prefix];
        $iter = $this->getIterator('ListObjects', $params);
        if ($regex) {
            $iter = \WP_Media_Folder\Aws\filter($iter, function ($c) use($regex) {
                return preg_match($regex, $c['Key']);
            });
        }
        return \WP_Media_Folder\Aws\S3\BatchDelete::fromIterator($this, $bucket, $iter, $options)->promise();
    }
    /**
    ...
    public function uploadDirectoryAsync($directory, $bucket, $keyPrefix = null, array $options = [])
    {
        ...
        return (new \WP_Media_Folder\Aws\S3\Transfer($this, $directory, $d, $options))->promise();
    }
    /**
    ...
    public function downloadBucketAsync($directory, $bucket, $keyPrefix = '', array $options = [])
    {
        ...
        return (new \WP_Media_Folder\Aws\S3\Transfer($this, $s, $directory, $options))->promise();
    }
    /**
    ...
    public function determineBucketRegionAsync($bucketName)
    {
        ...
        return $handler($command)->then(static function (\WP_Media_Folder\Aws\ResultInterface $result) {
            return $result['@metadata']['headers']['x-amz-bucket-region'];
        }, function (\WP_Media_Folder\Aws\Exception\AwsException $e) {
    ...
    private function determineBucketRegionFromExceptionBody(\WP_Media_Folder\Psr\Http\Message\ResponseInterface $response)
    {
        try {
     ...
     */
    private function checkExistenceWithCommand(\WP_Media_Folder\Aws\CommandInterface $command)
    {
        try {
     ...
     */
    public abstract function execute(\WP_Media_Folder\Aws\CommandInterface $command);
    /**
