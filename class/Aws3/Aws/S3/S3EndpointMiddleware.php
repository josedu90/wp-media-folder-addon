...
class S3EndpointMiddleware
{
    ...
    public function __invoke(\WP_Media_Folder\Aws\CommandInterface $command, \WP_Media_Folder\Psr\Http\Message\RequestInterface $request)
    {
    ...
    private static function isRequestHostStyleCompatible(\WP_Media_Folder\Aws\CommandInterface $command, \WP_Media_Folder\Psr\Http\Message\RequestInterface $request)
    {
        return \WP_Media_Folder\Aws\S3\S3Client::isBucketDnsCompatible($command['Bucket']) && ($request->getUri()->getScheme() === 'http' || strpos($command['Bucket'], '.') === false);
    }
    private function endpointPatternDecider(\WP_Media_Folder\Aws\CommandInterface $command, \WP_Media_Folder\Psr\Http\Message\RequestInterface $request)
    {
    ...
    private function canAccelerate(\WP_Media_Folder\Aws\CommandInterface $command)
    {
        return empty(self::$exclusions[$command->getName()]) && \WP_Media_Folder\Aws\S3\S3Client::isBucketDnsCompatible($command['Bucket']);
    }
    private function getBucketStyleHost(\WP_Media_Folder\Aws\CommandInterface $command, $host)
    {
    ...
    private function applyHostStyleEndpoint(\WP_Media_Folder\Aws\CommandInterface $command, \WP_Media_Folder\Psr\Http\Message\RequestInterface $request)
    {
        $uri = $request->getUri();
    ...
    private function applyDualStackEndpoint(\WP_Media_Folder\Aws\CommandInterface $command, \WP_Media_Folder\Psr\Http\Message\RequestInterface $request)
    {
    ...
    private function applyAccelerateEndpoint(\WP_Media_Folder\Aws\CommandInterface $command, \WP_Media_Folder\Psr\Http\Message\RequestInterface $request, $pattern)
    {
    ...
    private function getAccelerateHost(\WP_Media_Folder\Aws\CommandInterface $command, $pattern)
    {
        return "{$command['Bucket']}.{$pattern}.amazonaws.com";
    }
    private function getBucketlessPath($path, \WP_Media_Folder\Aws\CommandInterface $command)
    {
