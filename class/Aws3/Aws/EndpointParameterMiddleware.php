...
use WP_Media_Folder\Psr\Log\InvalidArgumentException;
/**
...
class EndpointParameterMiddleware
{
     ...
     * @return \Closure
     */
    public static function wrap(\WP_Media_Folder\Aws\Api\Service $service)
    {
        return function (callable $handler) use($service) {
            return new self($handler, $service);
        };
    }
    public function __construct(callable $nextHandler, \WP_Media_Folder\Aws\Api\Service $service)
    {
    ...
    public function __invoke(\WP_Media_Folder\Aws\CommandInterface $command, \WP_Media_Folder\Psr\Http\Message\RequestInterface $request)
    {
        ...
        if (!empty($operation['endpoint']['hostPrefix'])) {
            ...
            if (!\WP_Media_Folder\Aws\is_valid_hostname($host)) {
