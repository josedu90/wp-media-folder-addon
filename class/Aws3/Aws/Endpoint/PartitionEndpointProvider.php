...
use WP_Media_Folder\JmesPath\Env;
class PartitionEndpointProvider
{
    ...
    public function __construct(array $partitions, $defaultPartition = 'aws')
    {
        $this->partitions = array_map(function (array $definition) {
            return new \WP_Media_Folder\Aws\Endpoint\Partition($definition);
    ...
    {
        $data = \WP_Media_Folder\Aws\load_compiled_json(__DIR__ . '/../data/endpoints.json');
        $prefixData = \WP_Media_Folder\Aws\load_compiled_json(__DIR__ . '/../data/endpoints_prefix_history.json');
    ...
    public static function mergePrefixData($data, $prefixData)
    {
        ...
        foreach ($data["partitions"] as $index => $partition) {
            foreach ($prefixGroups as $current => $old) {
                $serviceData = \WP_Media_Folder\JmesPath\Env::search("services.{$current}", $partition);
                if (!empty($serviceData)) {
                    foreach ($old as $prefix) {
                        if (empty(\WP_Media_Folder\JmesPath\Env::search("services.{$prefix}", $partition))) {
