...
use WP_Media_Folder\Aws\Api\Validator;
...
use WP_Media_Folder\Aws\Endpoint\PartitionEndpointProvider;
use WP_Media_Folder\Aws\Signature\SignatureProvider;
use WP_Media_Folder\Aws\Endpoint\EndpointProvider;
use WP_Media_Folder\Aws\Credentials\CredentialProvider;
...
class ClientResolver
{
     ...
     */
    public function resolve(array $args, \WP_Media_Folder\Aws\HandlerList $list)
    {
        $args['config'] = [];
    ...
    public static function _apply_retries($value, array &$args, \WP_Media_Folder\Aws\HandlerList $list)
    {
        if ($value) {
            $decider = \WP_Media_Folder\Aws\RetryMiddleware::createDefaultDecider($value);
            $list->appendSign(\WP_Media_Folder\Aws\Middleware::retry($decider, null, $args['stats']['retries']), 'retry');
        }
    }
    public static function _apply_credentials($value, array &$args)
    {
        ...
        if ($value instanceof CredentialsInterface) {
            $args['credentials'] = \WP_Media_Folder\Aws\Credentials\CredentialProvider::fromCredentials($value);
        } elseif (is_array($value) && isset($value['key']) && isset($value['secret'])) {
            $args['credentials'] = \WP_Media_Folder\Aws\Credentials\CredentialProvider::fromCredentials(new \WP_Media_Folder\Aws\Credentials\Credentials($value['key'], $value['secret'], isset($value['token']) ? $value['token'] : null, isset($value['expires']) ? $value['expires'] : null));
        } elseif ($value === false) {
            $args['credentials'] = \WP_Media_Folder\Aws\Credentials\CredentialProvider::fromCredentials(new \WP_Media_Folder\Aws\Credentials\Credentials('', ''));
            $args['config']['signature_version'] = 'anonymous';
        } elseif ($value instanceof CacheInterface) {
            $args['credentials'] = \WP_Media_Folder\Aws\Credentials\CredentialProvider::defaultProvider($args);
        } else {
            throw new \InvalidArgumentException('Credentials must be an instance of ' . 'WP_Media_Folder\\Aws\\Credentials\\CredentialsInterface, an associative ' . 'array that contains "key", "secret", and an optional "token" ' . 'key-value pairs, a credentials provider function, or false.');
        }
    }
    public static function _apply_api_provider(callable $value, array &$args)
    {
        $api = new \WP_Media_Folder\Aws\Api\Service(\WP_Media_Folder\Aws\Api\ApiProvider::resolve($value, 'api', $args['service'], $args['version']), $value);
        if (empty($args['config']['signing_name']) && isset($api['metadata']['signingName'])) {
            $args['config']['signing_name'] = $api['metadata']['signingName'];
        }
        $args['api'] = $api;
        $args['parser'] = \WP_Media_Folder\Aws\Api\Service::createParser($api);
        $args['error_parser'] = \WP_Media_Folder\Aws\Api\Service::createErrorParser($api->getProtocol());
    }
    public static function _apply_endpoint_provider(callable $value, array &$args)
    {
        if (!isset($args['endpoint'])) {
            ...
            $result = \WP_Media_Folder\Aws\Endpoint\EndpointProvider::resolve($value, ['service' => $endpointPrefix, 'region' => $args['region'], 'scheme' => $args['scheme']]);
    ...
    public static function _apply_serializer($value, array &$args, \WP_Media_Folder\Aws\HandlerList $list)
    {
        $list->prependBuild(\WP_Media_Folder\Aws\Middleware::requestBuilder($value), 'builder');
    }
    public static function _apply_debug($value, array &$args, \WP_Media_Folder\Aws\HandlerList $list)
    {
        if ($value !== false) {
            $list->interpose(new \WP_Media_Folder\Aws\TraceMiddleware($value === true ? [] : $value));
        }
    }
    public static function _apply_stats($value, array &$args, \WP_Media_Folder\Aws\HandlerList $list)
    {
        ...
        if ($args['stats']['timer']) {
            $list->prependInit(\WP_Media_Folder\Aws\Middleware::timer(), 'timer');
        }
    }
    public static function _apply_profile($_, array &$args)
    {
        $args['credentials'] = \WP_Media_Folder\Aws\Credentials\CredentialProvider::ini($args['profile']);
    }
    public static function _apply_validate($value, array &$args, \WP_Media_Folder\Aws\HandlerList $list)
    {
        if ($value === false) {
            return;
        }
        $validator = $value === true ? new \WP_Media_Folder\Aws\Api\Validator() : new \WP_Media_Folder\Aws\Api\Validator($value);
        $list->appendValidate(\WP_Media_Folder\Aws\Middleware::validation($args['api'], $validator), 'validation');
    }
    public static function _apply_handler($value, array &$args, \WP_Media_Folder\Aws\HandlerList $list)
    {
        $list->setHandler($value);
    }
    public static function _default_handler(array &$args)
    {
        return new \WP_Media_Folder\Aws\WrappedHttpHandler(default_http_handler(), $args['parser'], $args['error_parser'], $args['exception_class'], $args['stats']['http']);
    }
    public static function _apply_http_handler($value, array &$args, \WP_Media_Folder\Aws\HandlerList $list)
    {
        $args['handler'] = new \WP_Media_Folder\Aws\WrappedHttpHandler($value, $args['parser'], $args['error_parser'], $args['exception_class'], $args['stats']['http']);
    }
    public static function _apply_user_agent($value, array &$args, \WP_Media_Folder\Aws\HandlerList $list)
    {
        if (!is_array($value)) {
            $value = [$value];
        }
        ...
        array_unshift($value, 'aws-sdk-php/' . \WP_Media_Folder\Aws\Sdk::VERSION);
        $args['ua_append'] = $value;
        $list->appendBuild(static function (callable $handler) use($value) {
            return function (\WP_Media_Folder\Aws\CommandInterface $command, \WP_Media_Folder\Psr\Http\Message\RequestInterface $request) use($handler, $value) {
                return $handler($command, $request->withHeader('User-Agent', implode(' ', array_merge($value, $request->getHeader('User-Agent')))));
            };
        });
    }
    public static function _apply_endpoint($value, array &$args, \WP_Media_Folder\Aws\HandlerList $list)
    {
    ...
    public static function _apply_idempotency_auto_fill($value, array &$args, \WP_Media_Folder\Aws\HandlerList $list)
    {
        $enabled = false;
        $generator = null;
        if (is_bool($value)) {
        ...
        if ($enabled) {
            $list->prependInit(\WP_Media_Folder\Aws\IdempotencyTokenMiddleware::wrap($args['api'], $generator), 'idempotency_auto_fill');
        }
    }
    public static function _default_endpoint_provider(array $args)
    {
        return \WP_Media_Folder\Aws\Endpoint\PartitionEndpointProvider::defaultProvider()->getPartition($args['region'], $args['service']);
    }
    public static function _default_serializer(array $args)
    {
        return \WP_Media_Folder\Aws\Api\Service::createSerializer($args['api'], $args['endpoint']);
    }
    public static function _default_signature_provider()
    {
        return \WP_Media_Folder\Aws\Signature\SignatureProvider::defaultProvider();
    }
    public static function _default_signature_version(array &$args)
    {
        ...
        $args['__partition_result'] = isset($args['__partition_result']) ? isset($args['__partition_result']) : call_user_func(\WP_Media_Folder\Aws\Endpoint\PartitionEndpointProvider::defaultProvider(), ['service' => $args['service'], 'region' => $args['region']]);
    ...
    public static function _missing_version(array $args)
    {
        ...
        $versions = \WP_Media_Folder\Aws\Api\ApiProvider::defaultProvider()->getVersions($service);
