...
{
    $version = (string) \WP_Media_Folder\GuzzleHttp\ClientInterface::VERSION;
    if ($version[0] === '5') {
        return new \WP_Media_Folder\Aws\Handler\GuzzleV5\GuzzleHandler();
    }
    if ($version[0] === '6') {
        return new \WP_Media_Folder\Aws\Handler\GuzzleV6\GuzzleHandler();
    }
...
function default_user_agent()
{
    $version = (string) \WP_Media_Folder\GuzzleHttp\ClientInterface::VERSION;
    if ($version[0] === '5') {
        return \WP_Media_Folder\GuzzleHttp\Client::getDefaultUserAgent();
    }
    if ($version[0] === '6') {
        return \WP_Media_Folder\GuzzleHttp\default_user_agent();
    }
    throw new \RuntimeException('Unknown Guzzle version: ' . $version);
}
/**
 ...
 */
function serialize(\WP_Media_Folder\Aws\CommandInterface $command)
{
    $request = null;
    $handlerList = $command->getHandlerList();
    // Return a mock result.
    $handlerList->setHandler(function (\WP_Media_Folder\Aws\CommandInterface $_, \WP_Media_Folder\Psr\Http\Message\RequestInterface $r) use(&$request) {
        $request = $r;
        return new \WP_Media_Folder\GuzzleHttp\Promise\FulfilledPromise(new \WP_Media_Folder\Aws\Result([]));
    });
