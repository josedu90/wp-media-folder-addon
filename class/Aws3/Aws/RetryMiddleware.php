...
class RetryMiddleware
{
    ...
    public static function createDefaultDecider($maxRetries = 3, $additionalRetryConfig = [])
    {
        ...
        return function ($retries, \WP_Media_Folder\Aws\CommandInterface $command, \WP_Media_Folder\Psr\Http\Message\RequestInterface $request, \WP_Media_Folder\Aws\ResultInterface $result = null, $error = null) use($maxRetries, $retryCurlErrors, $additionalRetryConfig) {
        ...
        $g = function ($value) use($handler, $decider, $delay, $command, $request, &$retries, &$requestStats, &$monitoringEvents, &$g) {
            ...
            if ($value instanceof \Exception || $value instanceof \Throwable) {
                if (!$decider($retries, $command, $request, null, $value)) {
                    return \WP_Media_Folder\GuzzleHttp\Promise\rejection_for($this->bindStatsToReturn($value, $requestStats));
                }
