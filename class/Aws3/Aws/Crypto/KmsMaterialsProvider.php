...
use WP_Media_Folder\Aws\Kms\KmsClient;
/**
 * Uses KMS to supply materials for encrypting and decrypting data.
 */
class KmsMaterialsProvider extends \WP_Media_Folder\Aws\Crypto\MaterialsProvider
{
    private $kmsClient;
    private $kmsKeyId;
    /**
     ...
     *                         and decrypting keys.
     */
    public function __construct(\WP_Media_Folder\Aws\Kms\KmsClient $kmsClient, $kmsKeyId = null)
    {
        $this->kmsClient = $kmsClient;
        $this->kmsKeyId = $kmsKeyId;
    }
    public function fromDecryptionEnvelope(\WP_Media_Folder\Aws\Crypto\MetadataEnvelope $envelope)
    {
        if (empty($envelope[\WP_Media_Folder\Aws\Crypto\MetadataEnvelope::MATERIALS_DESCRIPTION_HEADER])) {
            throw new \RuntimeException('Not able to detect kms_cmk_id from an' . ' empty materials description.');
        }
        $materialsDescription = json_decode($envelope[\WP_Media_Folder\Aws\Crypto\MetadataEnvelope::MATERIALS_DESCRIPTION_HEADER], true);
        ...
        return new \WP_Media_Folder\Aws\Crypto\KmsMaterialsProvider($this->kmsClient, $materialsDescription['kms_cmk_id']);
    }
    /**
