 ...
 */
class SignatureV4 implements \WP_Media_Folder\Aws\Signature\SignatureInterface
{
    use SignatureTrait;
     ...
     */
    public static function convertPostToGet(\WP_Media_Folder\Psr\Http\Message\RequestInterface $request)
    {
        if ($request->getMethod() !== 'POST') {
            throw new \InvalidArgumentException('Expected a POST request but ' . 'received a ' . $request->getMethod() . ' request.');
        }
        $sr = $request->withMethod('GET')->withBody(\WP_Media_Folder\GuzzleHttp\Psr7\stream_for(''))->withoutHeader('Content-Type')->withoutHeader('Content-Length');
    ...
    protected function getPayload(\WP_Media_Folder\Psr\Http\Message\RequestInterface $request)
    {
        ...
        if (!$request->getBody()->isSeekable()) {
            throw new \WP_Media_Folder\Aws\Exception\CouldNotCreateChecksumException('sha256');
        }
        try {
            return \WP_Media_Folder\GuzzleHttp\Psr7\hash($request->getBody(), 'sha256');
        } catch (\Exception $e) {
            throw new \WP_Media_Folder\Aws\Exception\CouldNotCreateChecksumException('sha256', $e);
        }
    }
    ...
    private function createPresignedRequest(\WP_Media_Folder\Psr\Http\Message\RequestInterface $request, \WP_Media_Folder\Aws\Credentials\CredentialsInterface $credentials)
    {
    ...
    private function parseRequest(\WP_Media_Folder\Psr\Http\Message\RequestInterface $request)
    {
        ...
        $uri = $request->getUri();
        return ['method' => $request->getMethod(), 'path' => $uri->getPath(), 'query' => \WP_Media_Folder\GuzzleHttp\Psr7\parse_query($uri->getQuery()), 'uri' => $uri, 'headers' => $request->getHeaders(), 'body' => $request->getBody(), 'version' => $request->getProtocolVersion()];
    }
    private function buildRequest(array $req)
    {
        if ($req['query']) {
            $req['uri'] = $req['uri']->withQuery(\WP_Media_Folder\GuzzleHttp\Psr7\build_query($req['query']));
        }
        return new \WP_Media_Folder\GuzzleHttp\Psr7\Request($req['method'], $req['uri'], $req['headers'], $req['body'], $req['version']);
    }
}
