...
use WP_Media_Folder\Aws\Exception\UnresolvedSignatureException;
/**
 * Signature providers.
 *
...
class SignatureProvider
{
    ...
    public static function resolve(callable $provider, $version, $service, $region)
    {
        ...
        throw new \WP_Media_Folder\Aws\Exception\UnresolvedSignatureException("Unable to resolve a signature for {$version}/{$service}/{$region}.\n" . "Valid signature versions include v4 and anonymous.");
    }
    /**
    ...
    public static function version()
    {
        return function ($version, $service, $region) {
            switch ($version) {
                case 's3v4':
                case 'v4':
                    return !empty(self::$s3v4SignedServices[$service]) ? new \WP_Media_Folder\Aws\Signature\S3SignatureV4($service, $region) : new \WP_Media_Folder\Aws\Signature\SignatureV4($service, $region);
                case 'v4-unsigned-body':
                    return new \WP_Media_Folder\Aws\Signature\SignatureV4($service, $region, ['unsigned-body' => 'true']);
                case 'anonymous':
                    return new \WP_Media_Folder\Aws\Signature\AnonymousSignature();
                default:
                    return null;
            }
        };
