...
class TraceMiddleware
{
    ...
    public function __invoke($step, $name)
    {
        ...
        return function (callable $next) use($step, $name) {
            return function (\WP_Media_Folder\Aws\CommandInterface $command, \WP_Media_Folder\Psr\Http\Message\RequestInterface $request = null) use($next, $step, $name) {
    ...
    private function commandArray(\WP_Media_Folder\Aws\CommandInterface $cmd)
    {
        return ['instance' => spl_object_hash($cmd), 'name' => $cmd->getName(), 'params' => $cmd->toArray()];
    }
    private function requestArray(\WP_Media_Folder\Psr\Http\Message\RequestInterface $request = null)
    {
    ...
    private function responseArray(\WP_Media_Folder\Psr\Http\Message\ResponseInterface $response = null)
    {
    ...
    private function streamStr(\WP_Media_Folder\Psr\Http\Message\StreamInterface $body)
    {
        return $body->getSize() < $this->config['stream_size'] ? (string) $body : 'stream(size=' . $body->getSize() . ')';
    }
    private function createHttpDebug(\WP_Media_Folder\Aws\CommandInterface $command)
    {
        if ($this->config['http'] && !isset($command['@http']['debug'])) {
            $command['@http']['debug'] = fopen('php://temp', 'w+');
        }
    }
    private function flushHttpDebug(\WP_Media_Folder\Aws\CommandInterface $command)
    {
