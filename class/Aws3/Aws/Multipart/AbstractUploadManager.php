...
namespace WP_Media_Folder\Aws\Multipart;

use WP_Media_Folder\Aws\AwsClientInterface as Client;
use WP_Media_Folder\Aws\CommandInterface;
use WP_Media_Folder\Aws\CommandPool;
use WP_Media_Folder\Aws\Exception\AwsException;
use WP_Media_Folder\Aws\Exception\MultipartUploadException;
 ...
 * @internal
 */
abstract class AbstractUploadManager implements \WP_Media_Folder\GuzzleHttp\Promise\PromisorInterface
{
    const DEFAULT_CONCURRENCY = 5;
    /** @var array Default values for base multipart configuration */
    private static $defaultConfig = ['part_size' => null, 'state' => null, 'concurrency' => self::DEFAULT_CONCURRENCY, 'prepare_data_source' => null, 'before_initiate' => null, 'before_upload' => null, 'before_complete' => null, 'exception_class' => 'WP_Media_Folder\\Aws\\Exception\\MultipartUploadException'];
     ...
     * @param array  $config
     */
    public function __construct(\WP_Media_Folder\Aws\AwsClientInterface $client, array $config = [])
    {
        $this->client = $client;
    ...
    public function promise()
    {
        ...
        return $this->promise = \WP_Media_Folder\GuzzleHttp\Promise\coroutine(function () {
            // Initiate the upload.
            ...
            if (!$this->state->isInitiated()) {
                ...
                $this->state->setStatus(\WP_Media_Folder\Aws\Multipart\UploadState::INITIATED);
            }
            ...
            $commands = new \WP_Media_Folder\Aws\CommandPool($this->client, $this->getUploadCommands($resultHandler), ['concurrency' => $this->config['concurrency'], 'before' => $this->config['before_upload']]);
            ...
            $this->state->setStatus(\WP_Media_Folder\Aws\Multipart\UploadState::COMPLETED);
     ...
     */
    protected abstract function handleResult(\WP_Media_Folder\Aws\CommandInterface $command, \WP_Media_Folder\Aws\ResultInterface $result);
    /**
    ...
    private function determineState()
    {
        ...
        $state = new \WP_Media_Folder\Aws\Multipart\UploadState($id);
    ...
    protected function getResultHandler(&$errors = [])
    {
        return function (callable $handler) use(&$errors) {
            return function (\WP_Media_Folder\Aws\CommandInterface $command, \WP_Media_Folder\Psr\Http\Message\RequestInterface $request = null) use($handler, &$errors) {
                return $handler($command, $request)->then(function (\WP_Media_Folder\Aws\ResultInterface $result) use($command) {
                    $this->handleResult($command, $result);
                    return $result;
                }, function (\WP_Media_Folder\Aws\Exception\AwsException $e) use(&$errors) {
                    $errors[$e->getCommand()[$this->info['part_num']]] = $e;
                    return new \WP_Media_Folder\Aws\Result();
                });
            };
        };
    }
