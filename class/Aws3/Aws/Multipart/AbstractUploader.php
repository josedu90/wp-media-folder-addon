...
abstract class AbstractUploader extends \WP_Media_Folder\Aws\Multipart\AbstractUploadManager
{
     ...
     * @param array  $config
     */
    public function __construct(\WP_Media_Folder\Aws\AwsClientInterface $client, $source, array $config = [])
    {
     ...
     */
    protected function limitPartStream(\WP_Media_Folder\Psr\Http\Message\StreamInterface $stream)
    {
        // Limit what is read from the stream to the part size.
        return new \WP_Media_Folder\GuzzleHttp\Psr7\LimitStream($stream, $this->state->getPartSize(), $this->source->tell());
    }
    ...
    private function determineSource($source)
    {
        ...
        if (is_string($source)) {
            $source = \WP_Media_Folder\GuzzleHttp\Psr7\try_fopen($source, 'r');
        }
        // Create a source stream.
        $stream = \WP_Media_Folder\GuzzleHttp\Psr7\stream_for($source);
