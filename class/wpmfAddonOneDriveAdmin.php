...
class WpmfAddonOneDriveAdmin extends WpmfAddonOneDrive
{
    ...
    public function createRootDriveFolder()
    {
        ...
        if (!empty($params['connected']) && !empty($params['onedriveBaseFolder']['id'])) {
            $exists = get_term_by('slug', 'onedrive', WPMF_TAXO);
            if (empty($exists)) {
                ...
                update_term_meta($exists->term_id, 'wpmf_drive_root_id', $params['onedriveBaseFolder']['id']);
            }
        }
    }
    ...
    public function addEditorAssets()
    {
        ...
        $params = array(
            ...
            'vars' => array(
                'block_cover' => WPMFAD_URL .'assets/blocks/wpmfonedrive/preview.png'
            )
        );
