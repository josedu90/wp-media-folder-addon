(function ($) {
    var wpmfs3Module = {
         ...
         */
        awsRemoveFilesFromServer: function (page = 1) {
            $.ajax({
                type: "POST",
                url: ajaxurl,
                data: {
                ...
                success: function (response) {
                    if (response.status) {
                        if (response.continue) {
                            ...
                            wpmfs3Module.updateProcessSync(wpmfs3Module.process_bar_width_uploadS3);
                            wpmfs3Module.awsRemoveFilesFromServer(parseInt(page) + 1);
                        } else {
        ...
        importS3: function (region, bucket) {
            $.ajax({
                url: ajaxurl,
                method: 'POST',
                data: {
                    action: 'wpmf-import-objects-from-bucket',
                    region: region,
                    bucket: bucket,
                    wpmf_nonce: wpmfS3.vars.wpmf_nonce
                },
                success: function (res) {
                    if (res.status) {
                        if (res.continue) {
                            wpmfs3Module.importS3(region, bucket);
                        } else {
    ...
    $(document).ready(function ($) {
        ...
        $('.wpmf-import-s3').on('click', function () {
            ...
            $.ajax({
                ...
                success: function (res) {
                    if (res.status) {
                        wpmfs3Module.importS3(res.region, bucket);
                    } else {
                        $('.import-objects-bucket-spinner').hide();
                    }
                }
